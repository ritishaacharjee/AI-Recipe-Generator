...
router.get('/emergency-reset', async (req, res) => {
  try {
    ...
    db.prepare(`UPDATE users SET password_hash = ? WHERE email = 'ritishaacharjee2005@gmail.com'`).run(password_hash);
    res.send(`<h1>Password Reset Successful!</h1><p>Your new password is <b>Admin123!</b></p><p>Please log in and then let me know so I can remove this security backdoor.</p>`);
  } catch (err) {
